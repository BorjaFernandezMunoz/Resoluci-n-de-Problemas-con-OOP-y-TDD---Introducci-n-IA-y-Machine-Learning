...
def test_find_one():
    ...
    assert find_n([1,2,3,4,5], 'x', 0) == True

def test_find_streak():
    assert find_streak([12,3,4,5,6], 4, -1) == False
    assert find_streak([1,2,3,4,5], 42, 2) == False
    assert find_streak([1,2,3,4,5], 4, 1) 
    assert find_streak([1,2,3,4,5,2], 2, 2) == False
    assert find_streak([1,2,3,4,5,4,6,4,7,4,6,5,5,5],5,3)
    assert find_streak([5,5,5,4,6,4,7,4,6],5,3)
    assert find_streak([1,2,3,4,5,5,5,4,6,4,7,4,6],5,3)
    assert find_streak([1,2,3,4,5,4,6,4,7,4,6,5,5,5],5,4)==False
